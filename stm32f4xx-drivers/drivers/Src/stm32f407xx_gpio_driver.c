...
void
GPIO_PeriClockControl(GPIO_RegDef_t* pGPIOx, uint8_t EnorDi) {
    if (EnorDi == ENABLE) {
        if (pGPIOx == GPIOA) {
            GPIOA_PCLK_EN();
...
void
GPIO_DeInit(GPIO_RegDef_t* pGPIOx) {
    if (pGPIOx == GPIOA) {
        GPIOA_REG_RESET();
    } else if (pGPIOx == GPIOB) {
        GPIOB_REG_RESET();
...
uint8_t
GPIO_ReadFromInputPin(GPIO_RegDef_t* pGPIOx, uint8_t PinNumber) {
    uint8_t value;
...
uint16_t
GPIO_ReadFromInputPort(GPIO_RegDef_t* pGPIOx) {
    uint16_t value;
...
void
GPIO_WriteToOutputPin(GPIO_RegDef_t* pGPIOx, uint8_t PinNumber, uint8_t Value) {
...
void
GPIO_WriteToOutputPort(GPIO_RegDef_t* pGPIOx, uint16_t Value) {
    pGPIOx->ODR = Value;
}
...
void
GPIO_IRQInterruptConfig(uint8_t IRQNumber, uint8_t EnorDi) {
    if (EnorDi == ENABLE) {
        if (IRQNumber <= 31) {
...
void
GPIO_IRQPriorityConfig(uint8_t IRQNumber, uint32_t IRQPriority) {
...
void
GPIO_IRQHandling(uint8_t PinNumber) {
